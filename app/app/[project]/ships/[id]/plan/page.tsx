...
export default async function PlanPage({
  ...
  const settingsHref = `/app/${project}/settings`;
    ...
    <>
      ...
      {recs.length === 0 && ship.status === "NEW" ? (
        ...
        <EmptyState
          ...
          actions={
            <Button variant="primary" icon="target" href={`/app/${project}/ships/new`}>
              Build a plan
            </Button>
          }
        />
        ...
        <>
          ...
          {paywall && lockedRecs.length > 0 && (
            <>
              <Note icon="lock" className="note-flow">
                {paywall} <Link href={settingsHref}>Upgrade to Pro</Link>
              </Note>
          ...
          {inLaunchMode ? (
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
              <Button variant="primary" icon="kit" href={`/app/${project}/ships/${ship.id}/kit`}>
